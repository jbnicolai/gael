...
var Emailer = function(templates, provider) {
  if (this instanceof Emailer === false) return new Emailer(provider);
...
Emailer.prototype.sendUsingTemplate = function(user, subject, name, data, force) {
  if ((!user.allowEmails && !force) || !user.email) return Promise.cast();
